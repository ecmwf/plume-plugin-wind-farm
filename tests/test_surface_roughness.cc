...
// Wires just enough real ModelData/WriteBackTracker machinery to call applyCoupling() for real — no Manager, no
...
CASE("test_golden_roughness_z0m_close_farm") {
    ...
    applyCouplingWithTracker(windFarm, sharedWindMap(), z0mField);
...
CASE("test_surface_roughness_respects_chunk_bounds") {
    ...
    EXPECT_THROWS_AS(applyCouplingWithTracker(windFarm, sharedWindMap(), z0mField), eckit::BadValue);
...
CASE("test_surface_roughness_increases_with_wind") {
    ...
    auto z0mAtWind = [&](double windU, double windV) {
        ...
        applyCouplingWithTracker(windFarm, windMap, z0mField);
